...
const CartModal = ({ cart, setCart, onClose }) => {
  ...
  // Extract the restaurant ID from the cart based on dishes
  useEffect(() => {
    const extractRestaurantId = () => {
      if (cart.length > 0) {
        ...
        if (uniqueRestaurantIds.length === 1) {
          setRestaurantId(uniqueRestaurantIds[0]); // Single restaurant in the cart
        } else if (uniqueRestaurantIds.length > 1) {
          setRestaurantId('Multiple IDs'); // Handle multiple restaurant IDs
        } else {
          setRestaurantId('None Found'); // No restaurant ID found
        }
      } else {
