...
const CartModal = ({ cart, setCart, onClose }) => {
  ...
  // Extract restaurant ID and customer ID on component load
  useEffect(() => {
    // Extract the customer ID from localStorage
    const storedCustomerId = localStorage.getItem('customer_id');
    if (storedCustomerId) {
      setCustomerId(storedCustomerId);
      console.log('Customer ID retrieved from localStorage:', storedCustomerId);
    } else {
  ...
  const handleOrder = () => {
    ...
    // Navigate to the Order Page with cart, customerId, and restaurantId
    navigate('/order', {
      state: {
        cart,
        customerId,
  ...
  return (
    <div className="cart-modal">
      <h2>Your Cart</h2>
      {restaurantId && <h3>Restaurant ID: {restaurantId}</h3>} {/* Display restaurant ID */}
