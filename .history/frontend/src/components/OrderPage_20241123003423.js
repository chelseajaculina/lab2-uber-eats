...
const OrderPage = () => {
  ...
  // Retrieve Customer ID and Restaurant ID on Mount
  useEffect(() => {
    // Retrieve customer ID from localStorage
    const storedCustomerId = localStorage.getItem('customer_id');
    if (storedCustomerId) {
      setCustomerId(storedCustomerId);
      console.log("Customer ID set from localStorage:", storedCustomerId);
    } else {
      setErrorMessage("Customer ID is missing. Please log in.");
      console.warn("Customer ID not found in localStorage.");
    }

    // Extract unique restaurant IDs from the cart
    if (cart.length > 0) {
      const uniqueRestaurantIds = [...new Set(cart.map((item) => item.restrestaurant_id))];
