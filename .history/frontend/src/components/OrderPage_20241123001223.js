...
const OrderPage = () => {
  ...
  // Fetch `restaurant_id` from the cart
  useEffect(() => {
    if (cart.length > 0) {
      const firstDishRestaurantId = cart[0]?.restaurant_id;
      const allRestaurantIds = [...new Set(cart.map((item) => item.restaurant_id))];

      if (allRestaurantIds.length > 1) {
        console.warn('Cart contains dishes from multiple restaurants:', allRestaurantIds);
        setErrorMessage('Your cart contains items from multiple restaurants. Please update your cart.');
      } else if (firstDishRestaurantId) {
        console.log('Setting restaurant_id:', firstDishRestaurantId);
