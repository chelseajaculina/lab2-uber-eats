...
const api = axios.create({
  baseURL: '/api', // Replace with the actual base URL of your backend API
  headers: {
...
const OrderPage = () => {
  ...
  useEffect(() => {
    // Fetch customer ID and restaurant ID from session or backend
    const fetchSessionData = async () => {
      try {
        const { data } = await api.get('/session-data/');
        setCustomerId(data.customer_id || null);
        setRestaurantId(cart[0]?.restaurant_id || data.restaurant_id || null);
        if (!data.customer_id || !cart[0]?.restaurant_id) {
          throw new Error('Customer ID or Restaurant ID is missing.');
        }
      } catch (error) {
