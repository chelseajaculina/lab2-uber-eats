...
const OrderPage = () => {
  ...
  // Set restaurant_id based on the first dish in the cart
  useEffect(() => {
    if (cart.length > 0) {
      const firstDishRestaurantId = cart[0]?.restaurant_id; // Assuming each dish has a `restaurant_id` field
      if (firstDishRestaurantId) {
        setRestaurantId(firstDishRestaurantId);
      } else {
        console.warn('No restaurant_id found in the first dish of the cart.');
        setErrorMessage('Restaurant ID is missing. Please check your cart.');
      }
    } else {
      console.warn('Cart is empty. Cannot determine restaurant_id.');
      setErrorMessage('Cart is empty. Please add items to your cart.');
    }
  }, [cart]);
