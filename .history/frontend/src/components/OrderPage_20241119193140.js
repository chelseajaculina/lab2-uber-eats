...
const OrderPage = () => {
  ...
  const [cart, setCart] = useState(location.state?.cart || []); // Retrieve cart from navigation state or fetch it dynamically
  ...
  useEffect(() => {
    ...
    // Fetch cart if not available in state
  ...
  const handlePlaceOrder = async () => {
    ...
    const address = selectedAddress
      ? selectedAddress
      : `${newAddress.street}, ${newAddress.apartment}, ${newAddress.city}, ${newAddress.state}, ${newAddress.zipcode}`;
