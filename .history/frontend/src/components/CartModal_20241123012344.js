...
const CartModal = ({ cart, setCart, onClose }) => {
  ...
  // Extract the restaurant ID from the first dish in the cart
  useEffect(() => {
    if (cart.length > 0) {
      const firstDish = cart[0];
      const restaurantIdFromFirstDish = firstDish.restaurant_id;
      console.log('Restaurant ID from the first dish:', restaurantIdFromFirstDish);

      if (restaurantIdFromFirstDish) {
        setRestaurantId(restaurantIdFromFirstDish);
      } else {
        setRestaurantId('Invalid Restaurant ID'); // Handle missing restaurant ID
      }
    } else {
