...
const OrderPage = () => {
  ...
  // Retrieve customer ID and restaurant ID
  useEffect(() => {
    ...
    setRestaurantId(cartRestaurantId || storedRestaurantId);

    if (!storedCustomerId) {
      console.warn('Customer ID not found in localStorage.');
      setErrorMessage('Customer ID is missing. Please log in again.');
    }
    if (!cartRestaurantId && !storedRestaurantId) {
      console.warn('Restaurant ID not found in cart or localStorage.');
      setErrorMessage('Restaurant ID is missing. Please select a restaurant.');
    }
  }, [cart]);
