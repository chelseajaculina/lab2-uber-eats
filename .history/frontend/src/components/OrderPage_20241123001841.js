...
const OrderPage = () => {
  ...
  // Debugging: Log cart data and customer_id
  useEffect(() => {
    console.group("Cart and Customer Debugging");
    console.log("Cart contents:", cart);

    // Check customer ID
    if (!customerId) {
      console.warn("Customer ID not found in localStorage. Setting error message.");
      setErrorMessage("Customer ID is missing. Please log in.");
    } else {
      console.log("Customer ID found:", customerId);
    }

    // Set restaurant ID based on cart
    if (cart.length > 0) {
