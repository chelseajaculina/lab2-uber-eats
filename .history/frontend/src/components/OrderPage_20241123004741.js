...
const OrderPage = () => {
  ...
  // Fetch Customer ID and Restaurant ID using API
  useEffect(() => {
    const fetchSessionData = async () => {
      try {
        ...
        const { data } = await axios.get('/api/session-info/', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });

        setCustomerId(data.customer_id); // Assuming 'customer_id' is returned by the API
        console.log('Customer ID fetched from API:', data.customer_id);
        ...
        if (cart.length > 0) {
          ...
          if (uniqueRestaurantIds.length === 1) {
            ...
            console.log('Restaurant ID fetched from cart:', uniqueRestaurantIds[0]);
