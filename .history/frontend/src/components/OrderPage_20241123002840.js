...
const OrderPage = () => {
  ...
  // Fetch session data and IDs
  useEffect(() => {
    const fetchUserData = async () => {
      const token = localStorage.getItem('token');
      if (!token) {
        setErrorMessage("User token is missing. Redirecting to login...");
        navigate("/login");
        return;
      }
      ...
      try {
        ...
        if (cart.length > 0) {
          ...
          if (uniqueRestaurantIds.length === 1) {
            ...
            console.log("Restaurant ID:", uniqueRestaurantIds[0]);
          } else {
            setErrorMessage("Cart contains items from multiple restaurants. Update your cart.");
          }
        } else {
          setErrorMessage("Your cart is empty. Please add items to your cart.");
        }
      } catch (err) {
        console.error("Error fetching session data:", err.response?.data || err.message);
        setErrorMessage("Failed to retrieve session data. Please log in again.");
      }
    };
  ...
  return (
    <div className="order-page">
      ...
      {customerName && <p className="customer-name">Welcome, {customerName}!</p>}
