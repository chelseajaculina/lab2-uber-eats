...
class OrderSerializer(serializers.ModelSerializer):
    class Meta:
        model = Order
        fields = ['id', 'restaurant', 'customer', 'dish', 'status', 'delivery_status', 'order_date']
        read_only_fields = ['restaurant', 'order_date']
        extra_kwargs = {'status': {'required': False, 'allow_null': True}, 
                        'delivery_status': {'required': False, 'allow_null': True}}

    def create(self, validated_data):
        # Remove 'restaurant' from validated_data since it will be assigned separately
        validated_data.pop('restaurant', None)
        restaurant = self.context['request'].user
        return Order.objects.create(restaurant=restaurant, **validated_data)
    
    def update(self, instance, validated_data):
        # Update order fields while managing image or specific updates
        for attr, value in validated_data.items():
            setattr(instance, attr, value)
        instance.save()
        return instance
