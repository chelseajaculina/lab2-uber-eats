...
# Order model (optional - assuming there will be order management)class OrderItem(models.Model):
...
class Order(models.Model):
    ...
    DELIVERY_ORDER_RECEIVED = 'Order Received'
    DELIVERY_PREPARING = 'Preparing'
    DELIVERY_ON_THE_WAY = 'On the Way'
    DELIVERY_PICK_UP_READY = 'Pick up Ready'
    ...
    delivery_status = models.CharField(max_length=20, choices=DELIVERY_STATUS_CHOICES, default=DELIVERY_ORDER_RECEIVED)
    order_date = models.DateTimeField(auto_now_add=True)
    total_cost = models.DecimalField(max_digits=10, decimal_places=2, default=0)

    def calculate_total(self):
        self.total_cost = sum(
            item.dish.price * item.quantity for item in self.order_items.all()
        )
        self.save()

    def __str__(self):
        return f"Order {self.id} - {self.customer.username} at {self.restaurant.restaurant_name}"
