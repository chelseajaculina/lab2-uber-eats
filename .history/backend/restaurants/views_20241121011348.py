...
class OrderView(APIView):
    ...
    def get(self, request):
        # Fetch orders for the authenticated restaurant
    ...
    def put(self, request, pk=None):
        # Update an order
        try:
            order = Order.objects.get(pk=pk, restaurant=request.user)
...
def create_order(request):
    try:
        ...
        dish_ids = data.get('dish_ids')  # List of dish IDs

        if not restaurant_id or not customer_name or not dish_ids:
