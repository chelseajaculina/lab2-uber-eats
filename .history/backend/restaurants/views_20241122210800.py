...
lass CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        Create a new order.
        """
        # Extract user and data from the request
        user = request.user
        data = request.data
        ...
        # Iterate through the items and ensure all dishes exist
        items = data['items']
        dishes = []
        for item in items:
            try:
                dish = Dish.objects.get(id=item['dish_id'], restaurant=restaurant)
                dishes.append({"dish": dish, "quantity": item['quantity']})
            except Dish.DoesNotExist:
                return Response(
                    {"error": f"Dish with ID {item['dish_id']} does not exist for the restaurant."},
                    status=status.HTTP_404_NOT_FOUND
                )
        ...
        try:
            ...
            order = Order.objects.create(
                ...
                customer=user,
                delivery_status='Order Received',  # Default status
                status='New',  # Default status
            )

            # Add dishes to the order
            for item in dishes:
                order.dish = item['dish']
                order.save()
