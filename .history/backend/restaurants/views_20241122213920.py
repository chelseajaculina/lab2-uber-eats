...
class CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Validate that all dishes exist and belong to the restaurant
            items = data['items']
            order_items = []
            for item in items:
                try:
                    dish = Dish.objects.get(id=item['dish_id'], restaurant=restaurant)
                    order_items.append({"dish": dish, "quantity": item['quantity']})
                except Dish.DoesNotExist:
                    return Response(
                        {"error": f"Dish with ID {item['dish_id']} does not exist in the restaurant."},
            ...
            # Link the dishes to the order
            for item in order_items:
                # Create a relationship for each dish
