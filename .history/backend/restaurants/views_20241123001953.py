...
from .models import Restaurant, Dish, OrderDish
...
class OrderDetailView(APIView):
    ...
    def get(self, request, order_id):
        try:
            # Fetch the order; ensure the requesting user is either the customer or associated with the restaurant
            order = Order.objects.get(
                id=order_id,
                restaurant__user=request.user  # Adjust based on how the relationship is defined
            )
    ...
    def put(self, request, order_id):
        try:
            order = Order.objects.get(
                id=order_id,
                restaurant__user=request.user  # Adjust based on user and restaurant relationship
            )
...
class CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        required_fields = ['restaurant_id', 'customer_id', 'address']
        ...
        try:
            customer = Customer.objects.get(id=data['customer_id'])  # Ensure correct customer_id is passed
        ...
        try:
            ...
            # Add the items (dishes) to the order
            for item in data['items']:
                try:
                    dish = Dish.objects.get(id=item['dish_id'], restaurant=restaurant)
                    OrderDish.objects.create(order=order, dish=dish, quantity=item['quantity'])
                except Dish.DoesNotExist:
                    return Response(
                        {"error": f"Dish with ID {item['dish_id']} does not exist in this restaurant."},
...
class SessionDataView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        """
        Retrieve session-related data such as customer_id and restaurant_id.
        """
        try:
            user = request.user  # Get the authenticated user
            customer_id = getattr(user, "customer_id", None)  # Assuming `customer_id` is a field on the user model

            # Example response (add more fields as necessary)
            session_data = {
            ...
            return Response(
                {"error": "Failed to retrieve session data.", "details": str(e)},
                status=500,
            )
